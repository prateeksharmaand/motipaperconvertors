...
export default function ExternalJobsPage() {
  ...
  async function handleExport() {
    setExporting(true);
    try {
      const res = await api.get("/admin/jobs", { params: { limit: 5000, order_type: "external" } });
  ...
  const { data, isLoading } = useQuery<PagedResult<Job>>({
    queryKey: ["external-jobs", actions.toParams()],
    queryFn: () => api.get("/admin/jobs", { params: { ...actions.toParams(), order_type: "external" } }).then((r) => r.data),
  ...
  const create = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); setShowForm(false); toast.success("Job card created successfully"); },
  ...
  const update = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); setEditing(null); toast.success("Job card updated successfully"); },
  ...
  const publish = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); setEditing(null); toast.success("Job card published"); },
  ...
  const createAndPublish = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); setShowForm(false); toast.success("Job card created and published"); },
    onError: () => toast.error("Failed to create and publish job card"),
  });

  const remove = useMutation({
    mutationFn: (id: string) => api.delete(`/admin/jobs/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); setDeleteConfirm(null); toast.success("Job card deleted"); },
  ...
  const changeStatus = useMutation({
    ...
    onSuccess: (_, { status }) => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); toast.success(`Status updated to ${status}`); },
  ...
  return (
    <div>
      ...
      <h1 style={{ marginBottom: 20 }}>External Job Cards</h1>
      {showForm && (
        <JobForm
          clients={clients}
          machines={machines}
          ...
          onCreateDraft={(form, papers) =>
            api.post("/admin/jobs", buildApiPayload(form, papers)).then(r => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); return r.data.id as string; })
          }
          onUpdateDraft={(id, form, papers) =>
            api.patch(`/admin/jobs/${id}`, buildPatchPayload(form, papers)).then(() => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); })
          }
          onPublish={(id, _form, _papers) =>
            api.patch(`/admin/jobs/${id}/status`, { status: "enquiry" }).then(() => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); setShowForm(false); })
          }
        ...
        <EditingJobFormWrapper
          ...
          onPublish={(id, form, papers) =>
            api.patch(`/admin/jobs/${id}`, buildPatchPayload(form, papers))
              ...
              .then(() => { qc.invalidateQueries({ queryKey: ["external-jobs"] }); setEditing(null); })
          }
