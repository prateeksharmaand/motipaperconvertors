...
// ── Top-level screen with In House / External tabs ────────
...
class _JobsViewState extends State<_JobsView> {
    ...
    return Scaffold(
      ...
      floatingActionButton: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
        if (_fabOpen) ...[
          Row(mainAxisSize: MainAxisSize.min, children: [
            Container(
              ...
              child: Text('New ${widget.orderType == 'external' ? 'External' : 'In House'} Job', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: Colors.white)),
            ),
            const SizedBox(width: 10),
            FloatingActionButton.small(
              heroTag: 'new-job-${widget.orderType}',
        ...
        FloatingActionButton(
          heroTag: 'jobs-main-${widget.orderType}',
          backgroundColor: AppColors.primary,
        ...
        return SafeArea(child: Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(child: OutlinedButton(
              ...
              style: OutlinedButton.styleFrom(side: BorderSide(color: sortDir == 'desc' ? AppColors.primary : AppColors.border, width: sortDir == 'desc' ? 2 : 1), backgroundColor: sortDir == 'desc' ? AppColors.primaryLight : null),
              child: Text('Newest First', style: TextStyle(color: sortDir == 'desc' ? AppColors.primary : AppColors.textPrimary)),
            )),
            const SizedBox(width: 12),
            Expanded(child: OutlinedButton(
              onPressed: () => setM(() => sortDir = 'asc'),
              style: OutlinedButton.styleFrom(side: BorderSide(color: sortDir == 'asc' ? AppColors.primary : AppColors.border, width: sortDir == 'asc' ? 2 : 1), backgroundColor: sortDir == 'asc' ? AppColors.primaryLight : null),
              child: Text('Oldest First', style: TextStyle(color: sortDir == 'asc' ? AppColors.primary : AppColors.textPrimary)),
            )),
          ]),
  ...
  void _showFilters(BuildContext ctx, JobsState state) {
    ...
    showModalBottomSheet(
      ...
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (_) => StatefulBuilder(builder: (context, setModal) => SafeArea(
